"""
Refactor: Moved helper functions from 'gistapi' to 'utils'.

This change enhances code clarity and readability.
"""
...
async def fetch_gist_content(session, gist: Gist, compiled_pattern):
    """
    Asynchronously fetches and searches each file content within
    a given gist for the compiled_pattern.
    Returns the gist URL if any file content matches the pattern,
    None otherwise.
    """
...
async def fetch_gists(
    ...
    """
    Paginates through all gists for a user and collects those
    that contain a match for the compiled_pattern.
    ...
    while max_page is None or page <= max_page:  # Pagination control
        ...
        async with session.get(gists_url, params=params) as response:
            if response.status == HTTPStatus.FORBIDDEN:
                """
                Rate limit detection; log and cease execution to prevent
                further limit exhaustion.
                """
